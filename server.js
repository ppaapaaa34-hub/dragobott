...
function inventoryState(user) {ensureRpg(user);return {items: user.rpgInventory.map(item => {const id = typeof item === "object" ? item.id : item;const catalogItem = byId.get(id);if(!catalogItem) return null;return typeof item === "object" ? { ...catalogItem, ...item } : catalogItem;}).filter(Boolean),equipped: user.equipped || {},totalCatalogItems: CATALOG.length};}
...
const exists = user.rpgInventory.some(item=>{if(typeof item==="object")return item.id===loot.id;return item===loot.id;});if(!exists){user.rpgInventory.push({id:loot.id,name:loot.name,icon:loot.icon,rarity:loot.rarity,slot:loot.slot,attack:loot.attack || 0,defense:loot.defense || 0,vitality:loot.vitality || 0});if(typeof user.markModified === 'function') user.markModified('rpgInventory');}else{loot=null;}} await persist(user); }
