...
function inventoryState(user) { ensureRpg(user); return { items: user.rpgInventory.map(id => byId.get(id)), equipped: user.equipped, totalCatalogItems: CATALOG.length }; }
...
app.post('/api/fight', async (req, res, next) => { try { const { telegramId } = req.body || {}; if (!validId(telegramId)) return res.status(400).json({ error: 'Некоректний ID' }); const user = await findOrCreate(Number(telegramId)); if (user.isBanned) return res.status(403).json({ error: 'Доступ заборонено' }); const stats = combatStats(user); const template = ENEMIES[Math.floor(Math.random() * Math.min(ENEMIES.length, 1 + Math.ceil(user.playerLevel / 6)))]; const scale = template.factor * (1 + Math.max(0, user.playerLevel - 1) * 0.16); const enemy = { ...template, hp: Math.round(72 * scale), attack: Math.round(8 * scale), reward: Math.round(120 * scale) };
  ...
  const win = heroHp > 0; let loot = null; if (win) { user.money += enemy.reward; user.playerXP += Math.round(enemy.reward * 0.65); while (user.playerXP >= user.playerLevel * 1000) { user.playerXP -= user.playerLevel * 1000; user.playerLevel++; } user.fightCount += 1; loot = rollLoot(user.playerLevel); if (loot && !user.rpgInventory.includes(loot.id)) user.rpgInventory.push(loot.id); else loot = null; await persist(user); }
  res.json({ success: true, win, heroHp: Math.max(0, heroHp), enemyHp: Math.max(0, enemyHp), rounds, reward: win ? enemy.reward : 0, enemy, level: user.playerLevel, xp: user.playerXP, money: user.money, loot, stats: combatStats(user) });
} catch (error) { next(error); } });
