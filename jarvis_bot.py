...
def handle_member_updates(message: types.ChatMemberUpdated):
    ...
    if old_status in ['left', 'kicked'] and new_status in ['member', 'administrator', 'restricted'] and not user.is_bot:
        ...
        if gender == 'Дівчина':
            greeting = f"Вітаємо в чаті, <b>{name}</b>! 🤍\Добро просимо до лав «Нової Банди»! 🎩
Правила прості: не флудити, поважати своїх і кидати якісні меми.
Заходь, вливайся та будь як удома! 😎"
        elif gender == 'Хлопець':
            greeting = f"Йо, <b>{name}</b>, вітаємо в чаті! 🤝\Сідай до нашого вогнища, скидай зброю. Вливайся в колектив, спілкуйся та будь як дома!"
        else:
            greeting = f"Вітаємо в нашій групі, <b>{name}</b>! 🤍\Напиши пару слів про себе або кидай улюблений мем, щоб одразу залетіти в тему!"
